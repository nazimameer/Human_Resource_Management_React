...
function LeaveApplications() {
  ...
  const handleApprove = (id)=>{
    axios.post('/hr/applicationApprove',{id:id}).then((response)=>{
  ...
  const handleReject = (id)=>{
    axios.post('/hr/applicationReject',{id:id}).then((response)=>{
      console.log(response)
      if(response.status === 200){
        axios
      .get("/hr/employee")
                        ...
                        <li className="relative flex p-6 mb-2 border-0 rounded-t-inherit rounded-xl bg-gray-50" key={index}>
                          ...
                          <div className="ml-auto text-right flex">
                            <div
                              ...
                              href="/"
                            >
                              <span className="text-lime-500" key={obj._id}  onClick={()=>handleApprove(obj._id)}> Approve</span>
                            </div>
                            <div
                              ...
                              href="/"
                            >
                              <span className="text-red-600" key={obj._id}  onClick={()=>handleReject(obj._id)}> Reject</span>
                            </div>
                          </div>
                        </li>
                      );
