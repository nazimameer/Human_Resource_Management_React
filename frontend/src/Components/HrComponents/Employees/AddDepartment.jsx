...
function AddDepartment({ open , closeModal, DepRefresh, Search }) {
  ...
  const handleSubmit = async (event) => {
    ...
    axios
      .post("/hr/checkDepartment", { name: name })
      .then((response) => {
        if (response.status === 200) {
          axios.post("/hr/addDepartment", formData).then((response) => {
      ...
      .catch((error) => {
        if (error.response.data.error) {
          message.error("Department Already Exist", [2]);
        }
      });
  };
  if(!open)return null;
